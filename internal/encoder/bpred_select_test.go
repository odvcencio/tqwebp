...
// TestDetailedBlockPreservesWholeBlockResult pins the conservative
// guarantee of the rule at record level. After a method 6 run, an
// unselected macroblock whose causal neighbourhood -- the macroblocks
// above and to the left its predictions read -- contains no adopted
// macroblock carries exactly the luma record its method 4 counterpart
// holds, and its 16x16 reconstruction area is byte-identical to the
// method 4 picture; raster-order coding legitimately diverges only
// downstream of an adoption. Smooth interior macroblocks, which the
// whole-block modes fit exactly, must never be adopted at all: a zero
// error cannot be halved.
func TestDetailedBlockPreservesWholeBlockResult(t *testing.T) {
	flat := image.NewRGBA(image.Rect(0, 0, 48, 32))
	for i := 0; i < len(flat.Pix); i += 4 {
		flat.Pix[i], flat.Pix[i+1], flat.Pix[i+2], flat.Pix[i+3] = 128, 128, 128, 0xff
	}
	gradient := image.NewRGBA(image.Rect(0, 0, 64, 32))
	for y := 0; y < 32; y++ {
		for x := 0; x < 64; x++ {
			c := uint8(x * 3)
			i := (y*64 + x) * 4
			gradient.Pix[i+0], gradient.Pix[i+1], gradient.Pix[i+2], gradient.Pix[i+3] = c, c, c, 0xff
		}
	}
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			enc4, _, _, err := encodeWithMethod(c.img, Config{Quality: 75, Method: 4})
			if err != nil {
				t.Fatalf("encode at method 4: %v", err)
			}
			enc6, _, selected, err := encodeWithMethod(c.img, Config{Quality: 75, Method: 6})
			if err != nil {
				t.Fatalf("encode at method 6: %v", err)
			}

			adopted := make([]bool, len(enc6.mbs))
			interiorAdopted := 0
			at := func(mbx, mby int) int { return mby*enc6.mbw + mbx }
			// neighborhoodAdopted reports whether any macroblock whose
			// reconstruction feeds this one's predictions -- the row
			// above from one column left through one column right, the
			// column immediately left, and the corner -- took the
			// B_PRED path. Only such neighbours can move an unselected
			// macroblock off the method 4 result; raster-order coding
			// makes every later macroblock legitimately diverge.
			neighborhoodAdopted := func(mbx, mby int) bool {
				for _, d := range [][2]int{{-1, -1}, {0, -1}, {1, -1}, {-1, 0}} {
					nx, ny := mbx+d[0], mby+d[1]
					if nx < 0 || ny < 0 || nx >= enc6.mbw {
						continue
					}
					if adopted[at(nx, ny)] {
						return true
			...
			for i := range enc6.mbs {
				if !enc6.mbs[i].bpred {
					mbx, mby := i%enc6.mbw, i/enc6.mbw
					if !neighborhoodAdopted(mbx, mby) {
						if got, want := enc6.mbs[i].yMode, enc4.mbs[i].yMode; got != want {
							t.Errorf("macroblock %d: unselected luma mode %v, method 4 chose %v", i, got, want)
						}
						if enc6.mbs[i].levels != enc4.mbs[i].levels || enc6.mbs[i].nz != enc4.mbs[i].nz {
							t.Errorf("macroblock %d: unselected record moved off the method 4 result", i)
						}
					}
					continue
				}
				adopted[i] = true
				selected--
				if enc6.mbs[i].nz[blockY2] {
					t.Errorf("macroblock %d marked the Y2 block nonzero", i)
				}
				for _, v := range enc6.mbs[i].levels[blockY2] {
					if v != 0 {
						t.Fatalf("macroblock %d holds Y2 levels in a B_PRED record", i)
					}
				}
				mbx, mby := i%enc6.mbw, i/enc6.mbw
				if !touchesFrameBorder(mbx, mby, enc6.mbw, enc6.mbh) && c.name != "mixed" {
					interiorAdopted++
				}
			}
			if selected != 0 {
				t.Fatalf("bookkeeping error: %d unaccounted selections", selected)
			}

			for mby := 0; mby < enc6.mbh; mby++ {
				for mbx := 0; mbx < enc6.mbw; mbx++ {
					i := at(mbx, mby)
					if adopted[i] || neighborhoodAdopted(mbx, mby) {
						continue
					}
					for r := 0; r < 16; r++ {
						y := mby*16 + r
						base := y*enc6.rec.YStride + mbx*16
						got := enc6.rec.Y[base : base+16]
						want := enc4.rec.Y[y*enc4.rec.YStride+mbx*16 : y*enc4.rec.YStride+mbx*16+16]
						if !bytes.Equal(got, want) {
							t.Fatalf("reconstruction of unselected macroblock %d moved off the method 4 picture", i)
						}
					}
				}
			}

			// On the smooth fixtures every interior macroblock fits a
			// whole-block mode exactly, so only border macroblocks --
			// which predict against the missing-sample fills -- may be
			// adopted.
			if c.name != "mixed" && interiorAdopted != 0 {
				t.Errorf("%d interior macroblocks adopted on smooth content", interiorAdopted)
			}
		})
	}
}
